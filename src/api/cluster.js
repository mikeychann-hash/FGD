...
export function initClusterRoutes(stateManager, npcSystem) {
  ...
  // Missing endpoints for backward compatibility
  // /data/cluster_status.json - called by dashboard.js:473
  router.get('/data/cluster_status.json', (req, res) => {
    try {
      res.json({
  ...
  // /data/metrics.json - called by dashboard.js:534
  router.get('/data/metrics.json', (req, res) => {
    try {
  ...
  router.get('/debug/bot/:id/view', authenticate, requirePermission('admin'), (req, res) => {
    ...
    const runtime =
      npcSystem.npcEngine.npcs instanceof Map
    ...
    const summary = {
      ...
      position:
        runtime?.position || registryBot?.lastKnownPosition || registryBot?.spawnPosition || null,
